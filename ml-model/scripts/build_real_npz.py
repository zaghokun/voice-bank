...
DATA_DIR = "../data"
REAL_AUDIO_DIR = os.path.join(DATA_DIR, "real_audio")
...
for intent in LABEL_CLASSES:
    folder = os.path.join(REAL_AUDIO_DIR, intent)
...
np.savez_compressed(os.path.join(DATA_DIR, "train_mfcc_real.npz"), features=X_train, labels=y_train)
np.savez_compressed(os.path.join(DATA_DIR, "dev_mfcc_real.npz"), features=X_dev, labels=y_dev)
np.savez_compressed(os.path.join(DATA_DIR, "test_mfcc_real.npz"), features=X_test, labels=y_test)

print("\n✅ Real NPZ selesai dibuat.")